...
brightness_slider:connect_signal('property::value', function()
  local brightness_level = tonumber(brightness_slider:get_value() / 100)

  spawn('xrandr --output eDP-1 --brightness ' .. math.max(brightness_level, 0.05), false)

  -- Update brightness osd
  awesome.emit_signal('module::brightness_osd', brightness_level * 100)
end)

brightness_slider:buttons(gears.table.join(awful.button({}, 4, nil, function()
...
local update_slider = function()
  awful.spawn.easy_async_with_shell(
      [[echo $(($(xrandr --verbose | awk '/Brightness/ { print $2; exit }') * 100)) | cut -d'.' -f 1]], function(stdout)
...
awesome.connect_signal('widget::brightness:update', function(value)
  brightness_slider:set_value(tonumber(value) * 100)
end)
