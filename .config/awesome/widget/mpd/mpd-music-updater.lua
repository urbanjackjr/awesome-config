...
local update_progress_bar = function()
	awful.spawn.easy_async_with_shell(
		[[
			playerctl position --format '{{ duration(mpris:length) }}'
		]], 
		function(stdout)
...
local check_if_playing = function()
	awful.spawn.easy_async_with_shell(
		[[
			playerctl status | grep -o Playing
		]],
		function(stdout)
...
local check_repeat_status = function()
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			if (stdout:match('Track') or stdout:match('Playlist')) then
...
vol_slider.vol_slider:connect_signal(
	...
	function()
		awful.spawn.easy_async_with_shell(
			'playerctl volume ' .. vol_slider.vol_slider:get_value(), 
			function() end
		)
	end
)
...
if [ -z $(pgrep playerctld) ]; then playerctld; fi
]]
